...
ds_grid_set_region(rooms_map2,0,0,rooms_hor,rooms_vert,0)
yy=round(rooms_vert/2)
xx=0
this_room=10//choose(10,7)
last_room=this_room
ds_grid_set(rooms_map2,xx,yy,this_room)
...
//hit=1
go=0
if xx+1<rooms_hor{if c=1{if((ds_grid_get(rooms_map2,xx+1,yy)=0)and(xx+1<rooms_hor)){hit=0 x_o+=1 go=1}else{hit+=1}}}
if xx-1>-1{if c=2{if ((ds_grid_get(rooms_map2,xx-1,yy)=0)and(xx-1>-1)){hit=0 x_o-=1 go=1}else{hit+=1}}}
if yy+1<rooms_vert{if c=3{if ((ds_grid_get(rooms_map2,xx,yy+1)=0)and(yy+1<rooms_vert)){hit=0 y_o+=1 go=1}else{hit+=1}}}
if yy-1>-1{if c=4{if ((ds_grid_get(rooms_map2,xx,yy-1)=0)and(yy-1>-1)){hit=0 y_o-=1 go=1}else{hit+=1}}}
if go=1
{
this_room=99
...
xx+=x_o
yy+=y_o
ds_grid_set(rooms_map2,xx,yy,this_room) i+=1 last_room=this_room
}
}
...
if x_o>0{this_room=9 make_end2()}if x_o<0{this_room=10 make_end2()}
if y_o>0{this_room=7 make_end2()}if y_o<0{this_room=8 make_end2()}
