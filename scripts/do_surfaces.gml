...
with(Static_light_object){draw_sprite_ext(spr_light,0,x-view_xview[0],y-view_yview[0],2,2,0,image_blend,1)}//light_draw()}
...
//with(xp_orb){draw_sprite_ext(spr_pixel,0,x+x_off-view_xview[0],y+y_off-view_yview[0],1,1,image_angle,image_blend,1)}
...
else
{
    if global.rage_on=0
        {
        draw_set_color(make_color_rgb(96,96,96)) // this one
        }
    else
        {
...
if 1=1//shader_enabled 
{
if global.can_use_shaders=1 {shader_set(shd_gaussian_horizontal);}
surface_set_target(final_surface);
draw_clear(c_black)
if global.can_use_shaders=1 {shader_set_uniform_f(uni_resolution_hoz, var_resolution_x, var_resolution_y);}
//draw_surface(global.light_surface,0,0);
draw_set_blend_mode_ext(bm_src_alpha, bm_inv_dest_alpha)
draw_surface(global.light_surface2,0,0);
draw_set_blend_mode(bm_normal)
if global.can_use_shaders=1 {shader_reset();}
surface_reset_target();
surface_set_target(global.light_surface2);
draw_clear(make_color_rgb(global.darkness,global.darkness,global.darkness))
//Do vertical blur last
if global.can_use_shaders=1 {shader_set(shd_gaussian_vertical);}
if global.can_use_shaders=1 {shader_set_uniform_f(uni_resolution_vert, var_resolution_x, var_resolution_y);}
draw_set_blend_mode_ext(bm_src_alpha,bm_inv_dest_alpha)
